...
class ARG_LLM {
    ...
    private static function generate_usernames_advanced( $count = 1, $params = array() ) {
        ...
        $system = "You are a helpful assistant that invents usernames for product reviews. About 50% should be realistic human names (first names, last names, or name + initial combinations like 'John', 'Smith', 'Sarah K.', 'J. Martinez'). Use SPACES between name parts, NOT underscores. Use a WIDE variety of diverse first names - never repeat names like Luna, Emma, or other common names. The other 50% MUST be EXTREMELY creative and varied usernames - NEVER repeat common words like 'code', 'star', 'tech', 'user', 'gamer', 'pro', 'master', 'ninja', 'guru'. Every creative username must be completely unique and unpredictable. Use diverse themes: animals, foods, hobbies, weather, emotions, random word combos, etc. NEVER use the name 'Luna' in any form. Avoid profanity and personal data. Return exactly one valid JSON array of strings and nothing else, e.g. [\"John Smith\", \"pizza_lover_88\", \"L.Ross\", \"Sarah K.\", \"WindyDay42\"]. Do NOT include extra commentary or explanation.";
        ...
        $user_msg .= "\nTask: Generate a single JSON array with exactly " . intval( $candidate_count ) . " unique candidate usernames. IMPORTANT: About 50% should be realistic human names (first names, last names, initials like \"John\", \"Smith\", \"Sarah K.\", \"J. Martinez\"). CRITICAL: Use SPACES between name parts (\"John Smith\" NOT \"John_Smith\"). Use a WIDE variety of first names - never use Luna, Emma, or repeat any name. Make every name unique and diverse. The other 50% MUST be WILDLY creative and unpredictable usernames - BANNED WORDS: code, star, tech, user, gamer, pro, master, ninja, guru, cyber, digital, cool, awesome. Use diverse creative themes like animals (PenguinDance), food (TacoLover99), nature (OceanBreeze), hobbies (BookwormJen), random combos (PurpleUmbrella), emotions (JoyfulSoul), objects (LampPost42), etc. Every creative username must be totally different from others. Requirements: all usernames must be 3-30 characters, use only letters, numbers, underscores, hyphens, dots, or spaces, and must not repeat supplied examples. Do NOT output nested arrays or trailing commas; if you cannot meet constraints, return an empty array []. Output only the JSON array.";
        $body = array(
            'model' => $model,
            'messages' => array(
                array( 'role' => 'system', 'content' => $system ),

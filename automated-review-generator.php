...
// Handle a quick HTTP transport check via admin-post.php
function arg_handle_check_http_transport() {
    if ( ! current_user_can( 'manage_options' ) ) {
        wp_die( esc_html__( 'Insufficient permissions', 'automated-review-generator' ) );
    }

    check_admin_referer( 'arg_check_http_action', 'arg_check_http_nonce' );

    // Try a small GET to a known reliable host
    $resp = wp_remote_get( 'https://www.google.com/', array( 'timeout' => 5 ) );
    $ok = false;
    $msg = '';
    if ( is_wp_error( $resp ) ) {
        $ok = false;
        $msg = $resp->get_error_message();
    } elseif ( isset( $resp['response'] ) && intval( $resp['response']['code'] ) < 400 ) {
        $ok = true;
        $msg = esc_html__( 'HTTP transport working (GET to google.com)', 'automated-review-generator' );
    } else {
        $ok = false;
        $msg = isset( $resp['response']['code'] ) ? 'HTTP ' . intval( $resp['response']['code'] ) : esc_html__( 'No response', 'automated-review-generator' );
    }

    $redirect = wp_get_referer() ?: admin_url( 'admin.php?page=arg-admin' );
    $redirect = add_query_arg( 'arg_check_http', $ok ? '1' : '0', $redirect );
    $redirect = add_query_arg( 'arg_check_msg', urlencode( $msg ), $redirect );
    wp_redirect( $redirect );
    exit;
}
add_action( 'admin_post_arg_check_http_transport', 'arg_handle_check_http_transport' );
